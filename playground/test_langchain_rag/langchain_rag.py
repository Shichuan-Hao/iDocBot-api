...
# # New API Docs: https://reference.langchain.com/python/langchain-core/documents/base/Document
document= loader.load()
# print(document)

# Chunking 切分
# Step 4. 使用 RecursiveCharacterTextSplitter 将内容分割成更小的块，这有助于通过将长文本分解为可管理的大小并有一些重叠来保留上下文来管理长文本。
# chunkvlz.up.rallway.app
# chunk_size
# chunk_overlap：上下文重叠的部分
"""
...
# for document_index, document_splits in enumerate(splits):
#     print(f"Document {document_index + 1}: ") # 显示文档编号
#     for split_index, split_text in enumerate(document_splits):
...
## Step 5. 对于每个块，构建 Embedding。智谱的 GLM Embedding 在线模型：https://open.bigmodel.cn/dev/api#text_embedding
## #GLM 4 官方调用说明：https://open.bigmodel.cn/dev/api#vector
# embeddings = []
#
# client = ZhipuAI()
#
# for document_splits in splits:
#     for split_type, split_content in document_splits:
...
#     print(f"Embedding {i + 1}: {embeddings[:3]}...") # 仅展示前10个值以示例


# step 6. Chroma 使用 GLM 4 的 Embedding 模型 提供的嵌入从这些块创建向量存储，从而促进高效检索。
...
##### 上面已经完成了 store，已经处理完所有的 index 过程
...
# prompt = hub.pull("rlm/rag-prompt")  此行会报错
langsmith_client = Client()
prompt = langsmith_client.pull_prompt("rlm/rag-prompt", dangerously_pull_public_prompt=True)
# print(prompt)
